...
/*
        public static Boolean showInstalledHidden(int enablepreview)
        {
            UpdateSession uSession = new UpdateSession();
            IUpdateSearcher uSearcher = uSession.CreateUpdateSearcher();
            // Hidden is a property, but are there really any?
            try
            {
                ISearchResult sResult = uSearcher.Search("IsInstalled=1 And IsHidden=1");
                Console.WriteLine("Found " + sResult.Updates.Count + " HIDDEN update(s)" + Environment.NewLine);
                foreach (IUpdate update in sResult.Updates)
                {
                    Console.WriteLine(update.Title);
                }
